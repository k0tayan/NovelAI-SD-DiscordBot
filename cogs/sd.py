...
class StableDiffusionCog(commands.Cog):
    def __init__(self, bot):
        ...
        elemental_code_jsons = [
            config["ELEMENTAL_CODE_JSON_PATH"] + "/" + i
            for i in os.listdir(config["ELEMENTAL_CODE_JSON_PATH"])
        ]
        self.elemental_code = []
        ...
        try:
            try:
                ...
                if error_message == "INVALID_OPTION":
                    await ctx.reply(
                        user_locale["ERROR"]["INVALID_OPTION"].replace(
                            "<option>", error["args"]["option"]
                        )
                    )
            ...
            if prompt.steps != config["STEPS"][2]:
                reply_message += "\n" + random.choice(
                    user_locale["MESSAGE"]["STEPS"]
                ).replace("<0>", str(prompt.steps))
            if prompt.translate:
    ...
    async def generate_with_ele(self, ctx: commands.Context):
        ...
        try:
            ...
            negative_prompt = (
                element["negative_prompt"].replace("{", "(").replace("}", ")")
            )
